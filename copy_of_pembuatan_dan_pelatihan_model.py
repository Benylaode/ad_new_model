...
Dataset ini berisi informasi tentang interaksi pengguna dengan iklan. Masing-masing baris mewakili satu interaksi, dengan kolom-kolom berikut:

user_id: ID unik setiap pengguna.

timestamp: Waktu interaksi terjadi.

device_type: Jenis perangkat yang digunakan (seperti Mobile, Desktop, atau Tablet).

location: Lokasi geografis pengguna (misalnya USA, UK, atau Canada).

age_group: Kelompok usia pengguna (misalnya 18-24, 25-34).

gender: Jenis kelamin pengguna (Male atau Female).

ad_id: ID unik untuk iklan.

content_type: Jenis konten dalam iklan (Text, Image, atau Video).

ad_topic: Topik atau kategori iklan (misalnya Fashion, Electronics).

ad_target_audience: Sasaran utama iklan (misalnya Tech Enthusiasts, Fitness Lovers).

click_through_rate: Rasio klik terhadap tayangan iklan (CTR).

conversion_rate: Rasio klik yang berujung pada aksi yang diinginkan, seperti pembelian.

engagement_level: Jenis interaksi pengguna dengan iklan (seperti Like, Share, Comment).

view_time: Lama waktu pengguna melihat iklan (dalam detik).

cost_per_click: Biaya yang dibayar pengiklan per klik.

performance



ini link nya https://www.kaggle.com/datasets/ziya07/advertising-campaign-dataset
